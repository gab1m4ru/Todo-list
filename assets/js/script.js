...
// Counting total and ready tasks
const totalTasksCounter = document.getElementById('total-counter')
const readyTasksCounter = document.getElementById('ready-counter')
// Function to update counters
const updateCounters = () => {
  const totalTasks = todoList.children.length // Count all tasks in the list
  const readyTasks = todoList.querySelectorAll('input[type="checkbox"]:checked').length // Count completed tasks

  totalTasksCounter.setAttribute('data-count', totalTasks.toString()) // Update total counter
  readyTasksCounter.setAttribute('data-count', readyTasks.toString() + ' of ' + totalTasks) // Update ready counter
}

// Creating task
createBtn.addEventListener('click', () => {
  const taskText = taskInput.value.trim(); // Get user input and trim whitespace
	const noTaskDisplay = document.getElementById('no-tasks')
	const countersBorder = document.querySelector('.tasks__counters')
  ...
  // Append elements to construct the task
  ...
  taskInput.value = ''
    ...
    if (todoList.children.length === 0) {
      ...
      if (countersBorder) countersBorder.style.borderBottom = ''
    }
  ...
  if (todoList && todoList.children.length > 0) {
    noTaskDisplay.style.display = 'none' // Hides no tasks message when user creates task
    if (countersBorder) countersBorder.style.borderBottom = 'none' // Also hides border line at counter
  }
